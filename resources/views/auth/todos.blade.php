<x-app-layout>
    ...
    <form class="flex flex-col items-center text-center gap-5" action="{{ route('todos.store') }}" method="POST" >
        @csrf
        <input  class="relative top-5 rounded-md font-sans shadow-sm bg-white p-1 h-12 w-72 transition duration-100" type="text" name="todo_name" id="todo_name" placeholder="Ajouter une nouvelle tâche" required> <br><br>
